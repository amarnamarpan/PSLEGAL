...
def remove_proper_nouns_and_prepositions(words):
		tagged_tokens = pos_tag(words)
		changed_tokens = [word for word, pos in tagged_tokens if pos != 'NNP' and pos != 'NNPS' and pos != 'IN']  # proper nouns,and prepositions
...
def process_content(sample_text):
	...
	for line in lines:
		...
		for unigram in unigrams:
			if all(c.isalpha() for c in unigram):	#Considering only alphabets
				fin_unigrams.append(unigram)
	tokenized=fin_unigrams
	tagged=[]
	try:
		for i in tokenized:
...
def get_list_of_proper_nouns(sample_text):

	tagged=process_content(sample_text)
	ctr=0
	pnouns_lst=[]
	for each in tagged:
		if each[1]=='NNP':
			pnouns_lst.insert(ctr,each[0].lower())
			ctr+=1
...
def my_tokenizer(text,gram,legal_stopwords):
	...
	#fin_unigrams = [stemmer.stem(token) for token in fin_unigrams]		#Uncomment if stemming is needed
...
def my_tokenizer1(text): # given a text document, this identifies the sentences and returns a list of tokenized sentences
	...
	for letter in text:
		if letter=='.':
			...
			new_text += letter
		curr_pos+=1
	sentences = [i.strip() for i in new_text.split('.')]	#breaking the documents into sentences based on remaining dots.
...
def extract(text):
	...
	legal_stopwords=open('legal_specific_stopwords.txt','r').read().split('\n')
	if '' in legal_stopwords:
		legal_stopwords.remove('')
	for i in range(len(legal_stopwords)):
		legal_stopwords[i] = legal_stopwords[i].lower()
	#Adding english stopwords from nltk corpus
	legal_stopwords=legal_stopwords + stopwords.words('english') ## Uncomment if you want to remove stopwords
	grammars=[]
	read_grammar_from_file=False

	if(read_grammar_from_file):
		lines=open('grammar_pos_tags.txt').read().split('\n')
		for line in lines:
			if line=='':
				continue
			grammars.append(form_grammar(line))
			grammars=[]
	...
	for grammar in grammars:
		for toks in sentences:
			...
			# filtering out terms that has only NN or NNs in it and the singlewords only NN|NNS|VBN|VBG
			NN_NNS_list=[]
			VBN_VBG_list=[]
			for tok in postoks:
			...
			for term in terms_for_this_loop:
				if len(term.split(' '))==1:	# This condition is true when the term is a singleword
